...
from pyairvisual.errors import (
    AirVisualError, InvalidKeyError, KeyExpiredError, LimitReachedError,
    NoStationError, NotFoundError, UnauthorizedError)
...
@pytest.mark.asyncio
async def test_call_limit_reached(
        aresponses, event_loop, fixture_limit_reached):
    """Test that the proper error is raised when the call limit is reached."""
    aresponses.add(
        'api.airvisual.com', '/v2/nearest_city', 'get',
        aresponses.Response(
            text=json.dumps(fixture_limit_reached), status=401))
...
async def test_city_not_found(aresponses, event_loop, fixture_city_not_found):
    ...
    aresponses.add(
        'api.airvisual.com', '/v2/nearest_city', 'get',
        aresponses.Response(
            text=json.dumps(fixture_city_not_found), status=401))
...
@pytest.mark.asyncio
async def test_incorrect_api_key(
        aresponses, event_loop, fixture_incorrect_api_key):
    """Test that the proper error is raised with an incorrect API key."""
    aresponses.add(
        'api.airvisual.com', '/v2/nearest_city', 'get',
        aresponses.Response(
            text=json.dumps(fixture_incorrect_api_key), status=401))
...
@pytest.mark.asyncio
async def test_no_nearest_station(
        aresponses, event_loop, fixture_no_nearest_station):
    """Test that the proper error is raised when no station is found."""
    aresponses.add(
        'api.airvisual.com', '/v2/nearest_station', 'get',
        aresponses.Response(
            text=json.dumps(fixture_no_nearest_station), status=401))
...
async def test_node_not_found(aresponses, event_loop, fixture_no_node):
    ...
    aresponses.add(
        'www.airvisual.com', '/api/v2/node/12345', 'get',
...
async def test_permission_denied(
        aresponses, event_loop, fixture_permission_denied):
    """Test that the proper error is raised when permission is denied."""
    aresponses.add(
        'api.airvisual.com', '/v2/nearest_station', 'get',
        aresponses.Response(
            text=json.dumps(fixture_permission_denied), status=401))
